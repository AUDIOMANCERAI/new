...
export const MpcPlayer: React.FC = () => {
  ...
  return (
      <div className="max-w-5xl w-full mx-auto bg-mpc-panel rounded border-[3px] border-[#333] shadow-panel overflow-hidden relative">
        ...
        {/* Top Metallic Bar */}
        <div className="h-16 bg-[#111] border-b-2 border-[#333] flex items-center px-6 justify-between relative z-10">
            <div className="flex items-center gap-4">
                <div className="w-2.5 h-2.5 rounded-full bg-mpc-danger shadow-[0_0_8px_currentColor] animate-pulse"></div>
                <div className="w-2.5 h-2.5 rounded-full bg-mpc-warning shadow-[0_0_8px_currentColor]"></div>
                <div className="w-2.5 h-2.5 rounded-full bg-mpc-success shadow-[0_0_8px_currentColor]"></div>
            </div>
            <div className="flex items-center gap-6">
                 <select 
                    value={kit.name}
                    onChange={handleKitSelect}
                    className="bg-black border-2 border-mpc-primary/30 text-xs font-bold text-mpc-primary px-3 py-1.5 rounded-sm focus:outline-none focus:border-mpc-primary focus:shadow-[0_0_10px_rgba(0,243,255,0.3)] uppercase tracking-wider font-mono cursor-pointer hover:bg-white/5 transition-colors"
                 >
                     {PRESET_KITS.map(k => (
        ...
        <div className="p-6 md:p-10 grid grid-cols-1 lg:grid-cols-12 gap-8 relative z-10">
            ...
            <div className="lg:col-span-5 flex flex-col gap-6">
                ...
                {/* AI Prompt Area */}
                <div className="bg-[#0e0e0e] p-5 rounded border-2 border-gray-800 shadow-lg">
                    ...
                    <div className="flex gap-2">
                        <input 
                            ...
                            className="flex-1 bg-black border-2 border-gray-700 rounded-sm px-3 py-2 text-xs font-bold focus:outline-none focus:border-mpc-accent focus:shadow-[0_0_10px_rgba(255,0,85,0.4)] text-white placeholder-gray-600 font-mono uppercase tracking-wide"
                            onKeyDown={(e) => e.key === 'Enter' && handleGenerate()}
                        />
                        <button 
                            onClick={handleGenerate}
                            disabled={isGenerating}
                            className="bg-mpc-accent hover:bg-white hover:text-black disabled:opacity-50 text-white p-2 rounded-sm transition-all shadow-[0_0_10px_rgba(255,0,85,0.3)] border-2 border-transparent"
                        >
                            <Wand2 size={18} />
                        </button>
                    </div>
                ...
                {/* Transport Controls */}
                <div className="bg-[#0e0e0e] p-6 rounded border-2 border-gray-800 flex flex-col gap-6 shadow-lg relative overflow-hidden">
                     {/* Decorative circuit line */}
                     <div className="absolute top-0 right-0 w-24 h-24 border-r-2 border-t-2 border-gray-800 rounded-tr-3xl pointer-events-none opacity-50"></div>
                        ...
                        <div className="flex items-center gap-6">
                            <button 
                                ...
                                className={`
                                    ...
                                    ${isPlaying 
                                        ? 'bg-mpc-success/20 border-mpc-success text-mpc-success shadow-[0_0_20px_rgba(0,255,157,0.4)]' 
                            ...
                            <button
                                ...
                                className={`
                                    ...
                                    ${!audioEngine.isRecordingSupported ? 'bg-black opacity-30 cursor-not-allowed border-gray-800' : 
                                      isRecording 
                                        ? 'bg-mpc-danger/20 border-mpc-danger text-mpc-danger animate-blink-red shadow-[0_0_20px_rgba(255,0,85,0.4)]' 
                            ...
                            <div className="flex flex-col ml-2">
                                ...
                                <div className="flex items-center gap-3">
                                    <input 
                                        ...
                                        className="w-24 h-1 bg-gray-700 rounded-lg appearance-none cursor-pointer accent-mpc-primary hover:accent-white transition-colors"
                                    />
                     ...
                     <div className="grid grid-cols-2 gap-4 mt-2">
                         <button onClick={clearPattern} className="flex items-center justify-center gap-2 bg-black border-2 border-gray-700 hover:border-mpc-danger hover:text-mpc-danger p-2.5 rounded-sm text-[10px] font-black uppercase transition-all tracking-wider hover:shadow-[0_0_10px_rgba(255,0,85,0.2)]">
                             <Trash2 size={14} /> Purge Memory
                         </button>
                         <button onClick={randomizePattern} className="flex items-center justify-center gap-2 bg-black border-2 border-gray-700 hover:border-mpc-primary hover:text-mpc-primary p-2.5 rounded-sm text-[10px] font-black uppercase transition-all tracking-wider hover:shadow-[0_0_10px_rgba(0,243,255,0.2)]">
            ...
            <div className="lg:col-span-7 flex flex-col gap-8">
                ...
                {/* Pads Grid */}
                <div className="bg-[#0e0e0e] p-6 rounded border-2 border-gray-800 shadow-xl relative backdrop-blur-md">
                    <div className="absolute top-3 left-4 text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                        <div className="w-1.5 h-1.5 bg-mpc-success rounded-full animate-pulse shadow-[0_0_5px_currentColor]"></div>
                        Trigger Matrix
                    </div>
                ...
                {/* Sequencer */}
                <div className="flex-1 bg-[#0e0e0e] p-5 rounded border-2 border-gray-800 relative shadow-lg">
                     <div className="flex justify-between items-center mb-4">
                        <div className="flex items-center gap-4 w-full justify-between">
                            <span className="text-[10px] font-black text-gray-500 uppercase tracking-widest flex items-center gap-2">
                                <div className="w-1.5 h-1.5 bg-mpc-primary rounded-full animate-pulse shadow-[0_0_5px_currentColor]"></div>
                                Sequence Data
                            </span>
                            <div className="flex items-center gap-1 bg-black border-2 border-gray-800 rounded px-2 py-1">
                                <button 
